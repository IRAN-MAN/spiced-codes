/*
Make a page that has a <textarea> element on it. As the user types visible characters 
into this field, the characters should be replaced with the characters in the corresponding 
position in the Gettysburg Address. (Note - you can get and set the text in a <textarea> 
    through its value property.)
*/

var speech = `Four score and seven years ago our fathers brought forth on this continent, 
...
textarea.addEventListener("input", teaxtareaHandler);

function teaxtareaHandler(event) {
    event.target.value = speech.substring(0, event.target.value.length);
}
